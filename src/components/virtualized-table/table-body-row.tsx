...
export function TableBodyRow({ rowVirtualizer, virtualRow, rowIdx }: TableBodyRowProps) {
  ...
  const oddCellBgColor = currentTheme === "light" ? "bg-gray-50" : "bg-picker-primary";
  ...
  const getRemoteUsersForCell = (colId: string): RemoteUser[] => {
    return remote.filter(
      (u) => u.selectedCell?.rowIndex === virtualRow.index && u.selectedCell?.colId === colId,
    );
  };
