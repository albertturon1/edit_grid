...
export function TableNumericalCell({
  ...
  function handleSelectionWithModifier() {
    ...
    for (let i = firstRowId; i < secondRowId; i++) {
      ...
      const isLoopedRowInOpposition = loopedRow?.getIsSelected() !== row.getIsSelected();
  ...
  return (
    ...
    >
      ...
      <h1 className={"mt-px text-xs sm:text-sm"}>
...
function calculateRowRange({ anchorRow, row }: { anchorRow: Row<TableRow>; row: Row<TableRow> }) {
  const selectedRowsFirstId = Number(anchorRow.id);
  const currentRowId = Number(row.id);

  const firstRowId =
    selectedRowsFirstId < currentRowId ? selectedRowsFirstId + 1 : currentRowId + 1;

  const secondRowId = selectedRowsFirstId < currentRowId ? currentRowId : selectedRowsFirstId;
