...
function prepareExportData(table: Table<TableRow>, rowSelectionMode: boolean) {
  ...
  const selectedRows = rowSelectionMode ? allRows.filter((row) => row.getIsSelected()) : allRows;
...
export function useExport() {
  ...
  const { selectedRows, visibleColumnNames } = prepareExportData(table, rowSelectionMode);
  ...
  function exportData(exportMode: ExportMode, filename: string, includeHeaders: boolean) {
    const csv = convertRowsToCsv(rows[exportMode], visibleColumnNames, includeHeaders);
